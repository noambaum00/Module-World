...
namespace music {
	...
	export function startMelodyInternal(melodyArray: string[], options: MelodyOptions) {
		init();
		const isBackground =
			options & (MelodyOptions.OnceInBackground | MelodyOptions.ForeverInBackground);
	...
	export function stopMelody(options: MelodyStopOptions) {
		if (options & MelodyStopOptions.Background) startMelody([], MelodyOptions.OnceInBackground);
		if (options & MelodyStopOptions.Foreground) startMelody([], MelodyOptions.Once);
	}
	...
	function playNextNote(melody: Melody): void {
		...
		for (let pos = 0; pos < currNote.length; pos++) {
			...
			switch (noteChar) {
				...
				default:
					prevNote = false;
					if (parsingOctave) currentOctave = parseIntSimple(noteChar);
			}
		}
		if (!parsingOctave) {
	...
	class Melody {
		...
		constructor(melodyArray: string[], options: MelodyOptions) {
			...
			this.background =
				options & (MelodyOptions.OnceInBackground | MelodyOptions.ForeverInBackground) ? 0xf0 : 0;
